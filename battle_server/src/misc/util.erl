...
-export([test/0,
		...
		reward_pack/1,
		 ...
		 random_reward/3,
		 is_in_time_new/2,
		 get_quick_sycee/1,name_to_switch/1,
		 get_item_list_without_bind/1
    ]).
...
%% 获取当前时间相对�10.1.1 Hour:00:00 的相对天�
get_relative_day(Hour) when is_integer(Hour) andalso Hour >= 0 andalso Hour =< 24 ->
...
is_in_time_new({{StartYear,StartMonth,StartDay},{StartHour,StartMin,StartSecond}}, 
			   {{EndYear,EndMonth,EndDay},{EndHour,EndMin,EndSecond}})->
	StarTime=calendar:datetime_to_gregorian_seconds({{StartYear,StartMonth,StartDay},{StartHour,StartMin,StartSecond}}),
	EndTime=calendar:datetime_to_gregorian_seconds({{EndYear,EndMonth,EndDay},{EndHour,EndMin,EndSecond}}),
	if
		StarTime<EndTime->
			Now  = calendar:local_time(),
			CurSecs = calendar:datetime_to_gregorian_seconds(Now),
			if
				CurSecs>=StarTime andalso EndTime>=CurSecs->true;
				true->false
			end;
		true->false
	end.
...
get_item_list_without_bind(List)->
	F=fun({Type,Num,_})->
			  {Type,Num} end,
	lists:map(F, List).
merge_three_arg_list(L1,List)->
	F=fun({Type,Num,Bind},L)->
			  case lists:keyfind(Type, 1, L) of
				  ...
				  {Type,New_Num,_}->lists:keyreplace(Type, 1, L, {Type,Num+New_Num,Bind})
			  end end,
	lists:foldl(F, L1, List).
%% for
%% @return {ok, State}
...
ip2str({A,B,C,D}) ->
	string:join([to_list(A),to_list(B),to_list(C),to_list(D)], ".").
...
%% 权重随机,返回随到的项和除去该项的剩余列表
random_by_weight_ex(List) ->
	F = fun({_Data, Weight}, Total) ->
			Total + Weight
		end,
	TotalWeight = lists:foldl(F, 0, List),
	random_by_weight_ex(List, rand:uniform() * TotalWeight, 0, []).
random_by_weight_ex([{Data, Weight}|Next], R, Total, PrevList) ->
	if 
		R =< Weight + Total ->
			{Data, lists:reverse(PrevList) ++ Next};
		true ->
			random_by_weight_ex(Next, R, Total + Weight, [{Data, Weight}|PrevList])
	end;
random_by_weight_ex([], _R, _Total, _PrevList) ->
	erlang:error(badarg).

shuffle(List) ->
	L1 = [{rand:uniform(),N}|| N <- List],
...
intersection(L1, L2) ->     	lists:filter(fun(Elem)->lists:member(Elem, L1) end, L2).
...
filter_duplicate(List) when is_list(List) ->
	RetList =
	lists:foldl(
		fun(Elem, Ret) ->
			case lists:member(Elem, Ret) of
...
get_tuple_list_from_str(Str) when erlang:is_list(Str)-> 
	get_list(Str);
get_tuple_list_from_str(Str) when erlang:is_binary(Str)->
	get_list(Str);
get_tuple_list_from_str(_Str)->[].
get_list(Str)->
		case to_list(Str) of
		[]->[];
...
get_str_from_tuple_list(List)->
	F=fun(Tuple)->
			  string:join([util:to_list(V) || V <- tuple_to_list(Tuple)],"|") end,
...
get_quick_sycee(Needitem) ->ok.
%%	lists:foldl(fun({Itemtype,Num},Acc) ->
%%						case data_item:get_data(Itemtype) of
%%							#st_item_type{quick_buy = Quick_buy} ->Quick_buy*Num+Acc;
%%							_ -> Acc
%%						end
%%				end, 0, Needitem).

name_to_switch(Name) when erlang:is_list(Name) ->
	unicode:characters_to_binary(Name);
name_to_switch(Name) ->Name.
syn_list([],L)->L;
syn_list([{Type,Num,_}|Next],L)->syn_list(Next,L++[{Type,Num}]).
	
random_reward(Uid,ChestId,1) ->
	get_random_reward(Uid,ChestId,1).
get_random_reward(_Uid,Chest_id,Num)->
	get_random_reward(0,Chest_id,Num,[]).
get_random_reward(_Prof,_Chest_id,0,L)->merge_three_arg_list([], L);
get_random_reward(Prof,Chest_id,Num,L) when Num>0->	
	Need=
		case fun_rand_box:drop_box(Chest_id,Prof) of
			List when length(List)>0->
				List;			
			_R->merge_three_arg_list([], L)
		end,
	get_random_reward(Prof,Chest_id,Num-1,L++Need).